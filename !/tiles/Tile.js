...
function Tile (key) {
	...
	this.bounds = new Bounds(sw.lon < ne.lon ? sw.lon : ne.lon, sw.lat, sw.lon < ne.lon ? ne.lon : sw.lon, ne.lat)
...
Tile.prototype.CreateOverlays = function (w, h) {
	...
	features.forEach(feature => {
		Feature.CreateOverlay(feature, this.Pixels, w, h)
		/*
		log(feature)
...
Tile.prototype.Download = function (path, w = 1024, h = 1024, type = 'satellite-v9', cb = () => {}) {
	let url = `https://api.mapbox.com/styles/v1/mapbox/${type}/static/[${this.bounds.s},${this.bounds.w},${this.bounds.n},${this.bounds.e}]/${w}x${h}?access_token=${process.env.MapboxApiToken}`
	log(url)
