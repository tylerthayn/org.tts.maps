(function (factory) {
	...
	let Range = function (min,max) {
		let range=[min,max]
		_.Define(range, 'min', {get: () => min}, true)
		_.Define(range,'max',{get:()=>max},true);return range}
	let util={
	...
	let Bounds = {
		LonLat: (minlon, minlat, maxlon, maxlat) => {
			...
			_.Define(bounds, 'min', {get: () => {return new Coordinate.LonLat(bounds.minlon, bounds.minlat)}}, true)
			_.Define(bounds, 'max', {get: () => {return new Coordinate.LonLat(bounds.maxlon, bounds.maxlat)}}, true)
	...
	_.Define(Tile, 'TileFromBounds', (bounds, zoomMax = 20, zoomMin = 0) => {
		let points = [[bounds[0], bounds[1]], [bounds[0], bounds[3]], [bounds[2], bounds[3]], [bounds[2], bounds[1]]]
		let tile = null, zoom = zoomMax

		while ((zoom >= zoomMin) && tile == null) {
			let tiles = _.uniq(points.map(point => {
				let _tile = GlobalMercator.LatLonToTile(point[1], point[0], zoom)
